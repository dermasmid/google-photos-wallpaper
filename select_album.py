...
def main():
    ...
    while not done:
        albums_data = service.albums().list(pageSize= 50, pageToken= next_page_token).execute()
    ...
    for album_index in albums_dict:
        print(f'{album_index}: {albums_dict[album_index]["title"]}    ({albums_dict[album_index]["url"]})')
